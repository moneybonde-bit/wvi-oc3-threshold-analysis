...
- `docs/opsi-threshold-per-indikator.html` — **Opsi Threshold**: satu kartu per indikator (semua 7) menampilkan keempat angka kandidat P1–P4 dan usulan threshold yang bisa dipilih (★). P2 memakai nilai best-performer AP tunggal yang nyata (bisa outlier)
...
- **Metadata threshold & rasional**: `Analisis_4_Pendekatan_Threshold_OC3_FINAL.xlsx` (sheet Data_Dasar, P1–P4, Summary_Rekomendasi, Triangulasi, Definisi_Threshold). Threshold usulan (mengikuti kolom "Pendekatan yang direkomendasikan" workbook): OIOS 22 → 10% (P1); OIOS 6 → 61,8% (P3); OIOS 7 → 41,0% (P3); OIOS 10 → 22,1% (P4); OIOS 157 → ±61,7% (P4, sementara); C2A.027994 → ±31,0% (P4, sementara); OIOS 27 → ditunda. P2 (Best Performer) memakai nilai AP terbaik tunggal yang nyata. Semua berlabel "usulan untuk didiskusikan".
